...
import styles from './cnt.scss';

interface ICnt {
  timeWork: number
}

export function Cnt({timeWork}: ICnt) {
  const timePomodoro = useSelector<RootState, number>(state => state.timer.pomodoro) / 60
  const pomodoros = Math.floor(timeWork / 60 / timePomodoro)

  return (
    <div className={styles.cnt}>
      {pomodoros === 0 &&
        <svg className={styles.svgBig} width="115" height="115" viewBox="0 0 115 115" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      {pomodoros !== 0 &&
        <div className={styles.pomodoroContainer}>
          <div className={styles.pomodoroBox}>
            <svg className={styles.svg} width="81" height="81" viewBox="0 0 81 81" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <div className={styles.pomodoroN}>{`x ${pomodoros}`}</div>
          </div>
          <div className={styles.pomodoroText}>{`${pomodoros} ${getNoun(pomodoros, 'помидор', 'помидора', 'помидоров')}`}</div>
        </div>
      }
    </div>
  );
